...
export function generateHTMLMarkup(
  ...
  const dropoffTime: string = moment(reservationStartTime)
    .format(timeFormat)
    ?.toUpperCase();
  const pickupTime: string = moment(reservationEndTime)
    .format(timeFormat)
    ?.toUpperCase();
  ...
  const start = reservationStartTime;
  const end = reservationEndTime;
  const href = `${hrefBase}/view/qr?startTime=${start}&endTime=${end}&qrcodeData=${qrcodeData}`;
...
export function formatBillingInfoForEmail(
  ...
  try {
    ...
    console.error("formatBillingInfoForEmail => error:", error);
    return "";
  }
...
export function generateDataForServer(data: DataForServer): string {
  try {
    ...
    console.error("generateDataForServer => error:", error);
    return "";
  }
